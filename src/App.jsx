...
const ROLES = {
  plant_manager: {
    ...
    tabs: ["overview", "gantt", "dispatch", "qc", "finance", "uptime", "routes", "users"],
    perms: { create: true, edit: true, delete: true, status: true, finance: true, qc: true, users: true, uptime: true, routes: true, allAssets: true, allTypes: true },
  },
  batch_operator: {
    id: "batch_operator",
  ...
  dispatcher: {
    ...
    tabs: ["gantt", "dispatch", "uptime", "routes"],
    perms: { create: true, edit: true, delete: false, status: true, finance: false, qc: false, users: false, allAssets: false, uptime: true, routes: true, allTypes: false, assetGroups: ["mt","pt"], jobTypes: ["Truck Dispatch","Concrete Pour","Maintenance"] },
  },
  qc_lab: {
    id: "qc_lab",
...
const BLANK = { id:null, assetId:"MT01", type:"Truck Dispatch", start:8, dur:2, status:"Scheduled", priority:"Normal", grade:"C30", volume:7, site:"", lat:"", lng:"", operator:"", notes:"" };
...
function Scheduler({ user, allUsers, setUsers, onLogout }) {
  ...
  const TAB_LABELS = { overview:"📊 Overview", gantt:"⬛ Timeline", dispatch:"☰ Dispatch", qc:"🔬 QC Data", finance:"💰 Finance", uptime:"⏱ Uptime", routes:"🗺 Routes", users:"👥 Users" };
      ...
      <div className="app">
        ...
        <div className="body-wrap">
          <main className="content">
            ...
            {/* ROUTES / MAP */}
            {tab === "routes" && (
              <RoutesTab jobs={jobs} allUsers={allUsers} role={role} />
            )}
              ...
              <MFld label="Site / Project Address">
                <input value={form.site}
                  onChange={e=>{
                    ...
                    const known = SITE_COORDS[site];
                    setForm(p=>({...p, site, lat: known ? String(known.lat) : p.lat, lng: known ? String(known.lng) : p.lng }));
                  }}
                  placeholder="e.g. Block 7 – Ortigas Ave"/>
              </MFld>
              {/* GPS Coordinates */}
              {(form.type === "Truck Dispatch" || form.type === "Concrete Pour") && (
                <div>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:5}}>
                    <span style={{fontFamily:"'Inter',sans-serif",fontSize:11,fontWeight:600,color:"#9ca3af",letterSpacing:"0.5px",textTransform:"uppercase"}}>GPS Coordinates</span>
                    {form.lat && form.lng && (
                  ...
                  <div className="form-2col">
                    <MFld label="Latitude">
                      <input value={form.lat||""}
                        ...
                        placeholder="e.g. 14.5866"
                        type="text" inputMode="decimal"/>
                    </MFld>
                    <MFld label="Longitude">
                      <input value={form.lng||""}
                        onChange={e=>setForm(p=>({...p,lng:e.target.value}))}
                        placeholder="e.g. 121.0566"
                        type="text" inputMode="decimal"/>
                    </MFld>
                  </div>
                  <div style={{fontSize:11,color:"#6b7280",marginTop:4}}>
                    💡 Find coordinates: open Google Maps → right-click the site → copy the numbers shown.
                    {SITE_COORDS[form.site] ? <span style={{color:"#4ade80",marginLeft:6}}>✓ Known site — auto-filled</span> : ""}
                  </div>
                </div>
              )}
              <div className="form-2col">
...
/* ═══════════════════════════════════════════════════════════════════════════
   ROUTES TAB — Leaflet map with delivery sites & truck routes
═══════════════════════════════════════════════════════════════════════════ */

// Known site coordinates (Metro Manila) — dispatcher can add lat/lng per job
const SITE_COORDS = {
  "Plant Yard":                   { lat: 14.5764, lng: 121.0851, label:"CSI Plant" },
  "Block 7 – Ortigas Ave":        { lat: 14.5866, lng: 121.0566, label:"Block 7 – Ortigas" },
  "Skyline Tower – Makati":       { lat: 14.5548, lng: 121.0244, label:"Skyline Tower" },
  "SM North – QC":                { lat: 14.6574, lng: 121.0322, label:"SM North QC" },
  "Pier 4 Footing – Port Area":   { lat: 14.5892, lng: 120.9731, label:"Pier 4 – Port Area" },
  "Plant Internal":               { lat: 14.5764, lng: 121.0851, label:"Plant Internal" },
};
const PLANT = { lat: 14.5764, lng: 121.0851, label:"CSI Batching Plant" };
...
function RoutesTab({ jobs, allUsers, role }) {
  ...
  // Get coords: job's own lat/lng → fallback to SITE_COORDS
  function getCoords(job) {
    if (job.lat && job.lng && !isNaN(+job.lat) && !isNaN(+job.lng))
      return { lat: +job.lat, lng: +job.lng, label: job.site };
    return SITE_COORDS[job.site] || null;
  }

  const dispatchJobs = jobs.filter(j =>
    (j.type === "Truck Dispatch" || j.type === "Concrete Pour") &&
    getCoords(j) !== null
  );

  const filtered = filterStatus === "All"
    ? dispatchJobs
    : dispatchJobs.filter(j => j.status === filterStatus);
  ...
  // Redraw markers/routes when jobs or filter changes
  useEffect(() => {
    if (leafRef.current) drawMap();
  }, [filtered, selJob]);
  ...
  function drawMap() {
    ...
    const plantIcon = L.divIcon({
      html: `<div style="background:#f59e0b;width:14px;height:14px;border-radius:50%;border:3px solid #fff;box-shadow:0 2px 6px #0008"></div>`,
      className: "", iconAnchor: [7,7]
    });
    L.marker([PLANT.lat, PLANT.lng], { icon: plantIcon })
      .addTo(map)
      .bindPopup(`<b style="font-family:Inter,sans-serif">🏭 ${PLANT.label}</b><br><span style="font-size:12px;color:#666">Batching Plant — Origin</span>`);

    // Delivery site markers + route lines
    const bounds = [[PLANT.lat, PLANT.lng]];
    filtered.forEach(job => {
      const coord = getCoords(job);
      if (!coord) return;
      const color     = STATUS_ROUTE_COLOR[job.status] || "#8b949e";
      const isSel     = selJob === job.id;
      const asset     = ALL_ASSETS.find(a => a.id === job.assetId);
      const op        = allUsers.find(u => u.id === job.operator);

      // Route line plant → site
      L.polyline([[PLANT.lat, PLANT.lng],[coord.lat, coord.lng]], {
        color, weight: isSel ? 4 : 2,
        opacity: isSel ? 1 : 0.55,
        dashArray: job.status === "Scheduled" ? "8,6" : null,
      }).addTo(map);

      // Site marker
      const siteIcon = L.divIcon({
        html: `<div style="background:${color};width:${isSel?16:11}px;height:${isSel?16:11}px;border-radius:50%;border:2px solid #fff;box-shadow:0 2px 8px #0009;transition:all .2s"></div>`,
        className: "", iconAnchor: [isSel?8:5.5, isSel?8:5.5]
      });
      const popup = `
        <div style="font-family:Inter,sans-serif;min-width:180px">
          <div style="font-weight:700;font-size:13px;margin-bottom:4px">${JOB_ICONS[job.type]} ${job.site}</div>
          <div style="font-size:12px;color:#555;margin-bottom:2px">🚛 ${asset?.name || job.assetId}</div>
          <div style="font-size:12px;color:#555;margin-bottom:2px">👷 ${op?.name || "Unassigned"}</div>
          <div style="font-size:12px;color:#555;margin-bottom:4px">🧱 ${job.grade} · ${job.volume}m³</div>
          <span style="font-size:11px;font-weight:700;padding:2px 7px;border-radius:3px;background:${color}22;color:${color}">${job.status}</span>
        </div>`;
      L.marker([coord.lat, coord.lng], { icon: siteIcon })
        .addTo(map)
        .bindPopup(popup);
  ...
  const stats = {
    total:      dispatchJobs.length,
    active:     dispatchJobs.filter(j => j.status === "In Progress").length,
    scheduled:  dispatchJobs.filter(j => j.status === "Scheduled").length,
    complete:   dispatchJobs.filter(j => j.status === "Complete").length,
    totalVol:   dispatchJobs.reduce((s,j) => s + j.volume, 0),
  };
    ...
    <div className="rt-wrap">
      ...
      <div className="rt-topbar">
        <div className="rt-kpis">
          {[
            { l:"Dispatch Jobs", v:stats.total,    c:role.color },
            { l:"Active",        v:stats.active,   c:"#f97316"  },
            { l:"Scheduled",     v:stats.scheduled,c:"#38bdf8"  },
            { l:"Complete",      v:stats.complete, c:"#4ade80"  },
            { l:"Total Volume",  v:`${stats.totalVol}m³`, c:"#c084fc" },
          ].map(k => (
            <div key={k.l} className="rt-kpi">
              <div className="rt-kpi-v" style={{color:k.c}}>{k.v}</div>
              <div className="rt-kpi-l">{k.l}</div>
            </div>
          ))}
        </div>
        <div className="rt-filters">
          {["All","In Progress","Scheduled","Complete","On Hold"].map(s => (
            <button key={s}
              className={`chip ${filterStatus===s?"active":""}`}
              style={filterStatus===s?{"--rc":role.color}:{}}
      ...
      <div className="rt-body">
        ...
        <div className="rt-map-col">
          {mapError ? (
            <div className="rt-map-err">
              <div style={{fontSize:32,marginBottom:12}}>🗺</div>
              <div style={{fontWeight:700,color:"#f0f6fc",marginBottom:6}}>Map unavailable</div>
              <div style={{fontSize:13,color:"#8b949e"}}>Could not load OpenStreetMap.<br/>Check your internet connection.</div>
            </div>
          ) : (
            <>
              {!mapReady && (
              ...
              <div ref={mapRef} className="rt-map" style={{opacity:mapReady?1:0}} />
            </>
          )}
          <div className="rt-legend">
            {Object.entries(STATUS_ROUTE_COLOR).map(([s,c]) => (
              <div key={s} className="rt-leg-item">
                <div style={{width:22,height:3,background:c,borderRadius:2,marginRight:5,flexShrink:0}}/>
                <span>{s}</span>
              </div>
            ))}
          </div>
        ...
        {/* JOB LIST */}
        <div className="rt-list-col">
          <div className="rt-list-hdr">Dispatch Jobs {filtered.length !== dispatchJobs.length && `(${filtered.length}/${dispatchJobs.length})`}</div>
          {filtered.length === 0 && (
            <div className="rt-empty">No dispatch jobs with known coordinates.</div>
          )}
          {filtered.map(job => {
            const asset = ALL_ASSETS.find(a => a.id === job.assetId);
            const op    = allUsers.find(u => u.id === job.operator);
            ...
            return (
              <div key={job.id}
                className={`rt-job-card${isSel?" rt-sel":""}`}
                style={{borderColor: isSel ? (STATUS_ROUTE_COLOR[job.status]+"aa") : "#21262d"}}
                onClick={() => { setSelJob(isSel ? null : job.id); }}>
                <div className="rt-jc-top">
                  <span style={{fontSize:14}}>{JOB_ICONS[job.type]}</span>
                  <div className="rt-jc-info">
                    <div className="rt-jc-site">{job.site}</div>
                    <div className="rt-jc-asset">
                      <span className="dot sm" style={{background:asset?.color}}/> {asset?.name}
                      {op && <span style={{color:"#6b7280",marginLeft:6}}>· {op.name}</span>}
                    </div>
                  </div>
                  <span className="s-badge" style={{color:sm.text,background:sm.bg,borderColor:sm.border,fontSize:10}}>
                    <span className="s-dot" style={{background:sm.dot}}/>{job.status}
                  </span>
                </div>
                <div className="rt-jc-meta">
                  <span className="g-badge" style={{fontSize:11}}>{job.grade}</span>
                  <span style={{color:"#9ca3af",fontSize:12}}>{job.volume}m³</span>
                  <span style={{color:"#9ca3af",fontSize:12}}>{fmtH(job.start)} · {job.dur}h</span>
                  {coord && (
                    <a href={`https://www.google.com/maps/dir/${PLANT.lat},${PLANT.lng}/${coord.lat},${coord.lng}`}
                      target="_blank" rel="noopener noreferrer"
                      className="rt-gmaps-btn"
          ...
          <div className="rt-note">
            <strong style={{color:"#f59e0b"}}>📍 Adding GPS coordinates:</strong><br/>
            When creating a job, enter Latitude &amp; Longitude in the form. Known sites auto-fill. For new sites: open Google Maps → right-click → copy the coordinates shown.
          </div>
        </div>
      </div>
    </div>
...
const BASE = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ROUTES TAB */
  .rt-wrap { display:flex; flex-direction:column; flex:1; overflow:hidden; }
  .rt-topbar { display:flex; align-items:center; justify-content:space-between; padding:10px 16px; background:#161b22; border-bottom:1px solid #21262d; gap:12px; flex-wrap:wrap; }
  .rt-kpis { display:flex; gap:20px; flex-wrap:wrap; }
  .rt-kpi { display:flex; flex-direction:column; }
  .rt-kpi-v { font-family:'Inter',sans-serif; font-size:20px; font-weight:800; line-height:1; }
  .rt-kpi-l { font-size:11px; color:#9ca3af; text-transform:uppercase; letter-spacing:0.5px; margin-top:2px; font-weight:500; }
  .rt-filters { display:flex; gap:5px; flex-wrap:wrap; }
  .rt-body { display:grid; grid-template-columns:1fr 300px; flex:1; overflow:hidden; }
  .rt-map-col { position:relative; overflow:hidden; border-right:1px solid #21262d; display:flex; flex-direction:column; }
  .rt-map { flex:1; width:100%; height:100%; min-height:400px; transition:opacity .3s; }
  .rt-map-loading { position:absolute; inset:0; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:12px; background:#0d1117; color:#8b949e; font-size:13px; z-index:10; }
  .rt-map-err { flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#0d1117; color:#8b949e; text-align:center; padding:40px; }
  .rt-spinner { width:28px; height:28px; border:3px solid #21262d; border-top-color:#f59e0b; border-radius:50%; animation:spin 0.8s linear infinite; }
  @keyframes spin { to { transform:rotate(360deg); } }
  .rt-legend { display:flex; gap:12px; padding:8px 14px; background:#161b22; border-top:1px solid #21262d; flex-wrap:wrap; }
  .rt-leg-item { display:flex; align-items:center; font-size:11px; color:#9ca3af; }
  .rt-list-col { overflow-y:auto; background:#0d1117; display:flex; flex-direction:column; }
  .rt-list-hdr { padding:10px 14px; font-size:11px; font-weight:600; letter-spacing:0.5px; color:#9ca3af; text-transform:uppercase; border-bottom:1px solid #21262d; background:#161b22; flex-shrink:0; }
  .rt-empty { padding:32px; text-align:center; color:#6b7280; font-size:13px; }
  .rt-job-card { padding:12px 14px; border-bottom:1px solid #21262d; border-left:3px solid transparent; cursor:pointer; transition:all .15s; }
  .rt-job-card:hover { background:#161b22; }
  .rt-sel { background:#161b22 !important; border-left-width:3px !important; }
  .rt-jc-top { display:flex; align-items:flex-start; gap:8px; margin-bottom:6px; }
  .rt-jc-info { flex:1; min-width:0; }
  .rt-jc-site { font-size:13px; font-weight:600; color:#f0f6fc; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .rt-jc-asset { font-size:11px; color:#6b7280; margin-top:2px; display:flex; align-items:center; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .rt-jc-meta { display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
  .rt-gmaps-btn { font-size:11px; font-weight:600; color:#38bdf8; text-decoration:none; padding:2px 7px; border:1px solid #38bdf844; border-radius:3px; background:#38bdf811; white-space:nowrap; margin-left:auto; }
  .rt-gmaps-btn:hover { background:#38bdf822; }
  .rt-note { padding:12px 14px; font-size:11px; color:#6b7280; line-height:1.5; border-top:1px solid #21262d; margin-top:auto; flex-shrink:0; }
  /* Leaflet popup override */
  .leaflet-popup-content-wrapper { border-radius:8px !important; box-shadow:0 4px 20px #0008 !important; }
