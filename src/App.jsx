...
/* ─── FONTS ─────────────────────────────────────────────────────────────── */
const GFONTS = `@import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=DM+Mono:wght@400;500&family=DM+Sans:wght@300;400;500;600&display=swap');`;
...
export default function App() {
  ...
  return user
    ? <Scheduler user={user} onLogout={() => setUser(null)} />
...
function Scheduler({ user, onLogout }) {
  ...
  if (userPane) return (
    <>
      ...
      <UsersPanel onBack={() => setUserPane(false)} />
    </>
  );
                ...
                <table className="tbl">
                  ...
                  <tbody>
                    {visible.map(job => {
                      ...
                      const op = USERS.find(u => u.id === job.operator);
                      return (
              ...
              <div className="sd-body" key={selJob.id}>
                ...
                <div className="kv-list">
                  {[
                    ...
                    ["Operator",   USERS.find(u=>u.id===selJob.operator)?.name || "—"],
                  ].map(([k,v]) => (
    ...
    <>
      ...
      <div className="app">
        <header className="topbar">
          <div className="tb-left">
            <div className="tb-chip">RMC OPS</div>
            <div><h1 className="tb-title">User Management</h1><div className="tb-sub">4 Process Owner Roles · Access Control</div></div>
          </div>
          <div className="tb-right">
            <button className="btn-ghost" onClick={onBack}>← Back to Scheduler</button>
          </div>
        </header>
        ...
        <div className="nav-bar">
          <div className="tab-list">
            <button className={`tab-btn ${fr==="all"?"active":""}`} onClick={()=>setFr("all")}>All Roles</button>
            {Object.values(ROLES).map(r=>(
              ...
              return (
                <div key={u.id} className="um-card" style={{ borderColor: r.color + "44" }}>
                  <div className="umc-top" style={{ background: r.color + "12" }}>
                    ...
                    <div>
                      <div className="umc-name">{u.name}</div>
                      <div className="umc-role" style={{ color: r.color }}>{r.icon} {r.label}</div>
                      {u.asset && <div className="umc-asset">Asset: {getAsset(u.asset)?.name}</div>}
                    </div>
                  </div>
...
const BASE = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: #0d1117; color: #c9d1d9; font-family: 'DM Sans', sans-serif; }
...
const LOGIN_CSS = `
  ...
  .login-brand {}
  .login-chip { font-family: 'DM Mono', monospace; font-size: 10px; font-weight: 500; letter-spacing: 4px; color: #f59e0b; background: #f59e0b14; border: 1px solid #f59e0b33; padding: 4px 10px; border-radius: 3px; display: inline-block; margin-bottom: 16px; }
  .login-h1 { font-family: 'Syne', sans-serif; font-size: 30px; font-weight: 800; color: #f0f6fc; line-height: 1.15; margin-bottom: 10px; }
  ...
  .lr-name { font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 700; margin-bottom: 2px; }
  ...
  .us-title { font-family: 'Syne', sans-serif; font-size: 26px; font-weight: 800; color: #f0f6fc; margin-bottom: 4px; }
  .us-sub { font-size: 13px; color: #8b949e; margin-bottom: 28px; }
  .role-section { margin-bottom: 24px; }
  .rs-label { font-family: 'DM Mono', monospace; font-size: 11px; font-weight: 500; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 10px; }
  ...
  .ut-av { width: 46px; height: 46px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-family: 'Syne', sans-serif; font-size: 14px; font-weight: 800; border: 2px solid; }
  ...
  .pin-av { width: 70px; height: 70px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-family: 'Syne', sans-serif; font-size: 20px; font-weight: 800; margin: 0 auto 12px; border: 2px solid; }
  .pin-name { font-family: 'Syne', sans-serif; font-size: 20px; font-weight: 800; color: #f0f6fc; margin-bottom: 3px; }
  ...
  .numkey { width: 68px; height: 68px; border-radius: 10px; border: 1px solid #30363d; background: #0d1117; color: #f0f6fc; font-family: 'Syne', sans-serif; font-size: 22px; font-weight: 700; cursor: pointer; transition: all .1s; }
...
const APP_CSS = `
  ...
  .tb-chip { font-family: 'DM Mono', monospace; font-size: 10px; font-weight: 500; letter-spacing: 3px; color: #f59e0b; background: #f59e0b14; border: 1px solid #f59e0b33; padding: 4px 9px; border-radius: 3px; white-space: nowrap; }
  .tb-title { font-family: 'Syne', sans-serif; font-size: 19px; font-weight: 800; color: #f0f6fc; line-height: 1; }
  .tb-sub { font-size: 11px; color: #8b949e; margin-top: 2px; }
  .tb-right { display: flex; align-items: center; gap: 9px; flex-wrap: wrap; }
  .tb-clock { font-family: 'DM Mono', monospace; font-size: 18px; color: #f0f6fc; letter-spacing: 1px; }
  .user-badge { display: flex; align-items: center; gap: 9px; padding: 5px 11px; border-radius: 8px; border: 1px solid; }
  .ub-av { width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 800; }
  .ub-name { font-size: 12px; font-weight: 600; color: #f0f6fc; line-height: 1; }
  .ub-role { font-size: 10px; font-weight: 600; margin-top: 1px; }
  .btn-primary { font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 700; letter-spacing: .5px; background: var(--rc, #f59e0b); color: #000; border: none; padding: 7px 15px; border-radius: 6px; cursor: pointer; transition: filter .15s; white-space: nowrap; }
  .btn-primary:hover { filter: brightness(1.15); }
  .btn-ghost { font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 600; background: transparent; color: #8b949e; border: 1px solid #30363d; padding: 6px 13px; border-radius: 6px; cursor: pointer; transition: all .15s; white-space: nowrap; }
  ...
  .rb-title { font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 800; letter-spacing: .3px; }
  .rb-desc { font-size: 11px; color: #8b949e; margin-top: 1px; }
  .rb-perms { display: flex; gap: 5px; flex-wrap: wrap; }
  .rb-perm { font-family: 'DM Mono', monospace; font-size: 10px; font-weight: 500; padding: 2px 7px; border-radius: 3px; }
  ...
  .sc-val { font-family: 'Syne', sans-serif; font-size: 24px; font-weight: 800; line-height: 1; }
  ...
  .tab-btn { font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 700; padding: 5px 13px; border-radius: 5px; border: 1px solid #30363d; background: transparent; color: #8b949e; cursor: pointer; transition: all .15s; white-space: nowrap; }
  ...
  .f-lbl { font-family: 'DM Mono', monospace; font-size: 10px; color: #8b949e; text-transform: uppercase; letter-spacing: 1px; }
  .chip { font-size: 11px; padding: 3px 8px; border-radius: 20px; border: 1px solid #30363d; background: transparent; color: #8b949e; cursor: pointer; transition: all .15s; white-space: nowrap; }
  ...
  .section-hdr { padding: 12px 16px; font-family: 'DM Mono', monospace; font-size: 11px; font-weight: 500; letter-spacing: 2px; color: #8b949e; text-transform: uppercase; border-bottom: 1px solid #21262d; background: #161b22; }
  /* OVERVIEW */
  ...
  .ov-group-hdr { padding: 9px 13px; font-family: 'DM Mono', monospace; font-size: 10px; letter-spacing: 2px; color: #8b949e; border-bottom: 1px solid #21262d; background: #0d1117; }
  ...
  .oa-status { font-family: 'DM Mono', monospace; font-size: 10px; font-weight: 500; letter-spacing: 1px; }
  ...
  .gt-grade { font-family: 'Syne', sans-serif; font-size: 18px; font-weight: 800; color: #f59e0b; }
  .gt-vol { font-family: 'Syne', sans-serif; font-size: 14px; font-weight: 700; color: #f0f6fc; margin-top: 3px; }
  ...
  .g-asset-col { width: 140px; flex-shrink: 0; padding: 6px 12px; font-family: 'DM Mono', monospace; font-size: 9px; color: #8b949e; letter-spacing: 2px; }
  .g-hr { flex: 1; text-align: center; font-family: 'DM Mono', monospace; font-size: 10px; color: #8b949e; padding: 6px 0; border-left: 1px solid #21262d; }
  .gantt-body { overflow-y: auto; flex: 1; }
  .g-grp-row { padding: 5px 12px; background: #0d1117; border-bottom: 1px solid #21262d; font-family: 'DM Mono', monospace; font-size: 9px; font-weight: 500; letter-spacing: 2px; color: #8b949e; text-transform: uppercase; }
  ...
  .g-asset-name { font-size: 11px; color: #c9d1d9; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  ...
  .g-now-lbl { position: absolute; top: 2px; left: 3px; font-family: 'DM Mono', monospace; font-size: 7px; color: #f97316; letter-spacing: 1px; }
  .g-bar { position: absolute; top: 4px; height: 30px; border-radius: 4px; border: 1px solid; cursor: grab; display: flex; align-items: center; gap: 4px; padding: 0 6px; font-size: 11px; font-weight: 600; white-space: nowrap; overflow: hidden; transition: filter .1s, top .1s; z-index: 2; }
  ...
  .g-bar-vol { font-family: 'DM Mono', monospace; font-size: 9px; opacity: .8; flex-shrink: 0; }
  /* TABLE */
  .table-wrap { overflow: auto; flex: 1; }
  .tbl { width: 100%; border-collapse: collapse; font-size: 12px; }
  .tbl thead tr { background: #161b22; border-bottom: 2px solid #21262d; position: sticky; top: 0; z-index: 2; }
  .tbl th { padding: 8px 10px; font-family: 'DM Mono', monospace; font-size: 9px; font-weight: 500; letter-spacing: 2px; color: #8b949e; text-align: left; white-space: nowrap; text-transform: uppercase; }
  .tbl td { padding: 7px 10px; border-bottom: 1px solid #21262d; vertical-align: middle; }
  ...
  /* BADGES */
  .g-badge { font-family: 'DM Mono', monospace; font-size: 11px; font-weight: 500; color: #f59e0b; background: #f59e0b14; padding: 2px 6px; border-radius: 3px; }
  .g-lg { font-size: 14px; padding: 3px 9px; }
  .p-badge { font-family: 'DM Mono', monospace; font-size: 10px; padding: 2px 6px; border-radius: 3px; font-weight: 500; }
  ...
  .gc-grade { font-family: 'Syne', sans-serif; font-size: 18px; font-weight: 800; color: #f59e0b; }
  .gc-vol { font-family: 'Syne', sans-serif; font-size: 16px; font-weight: 700; color: #f0f6fc; margin-top: 3px; }
  ...
  .ff-lbl { font-family: 'DM Mono', monospace; font-size: 10px; color: #8b949e; text-transform: uppercase; letter-spacing: 1px; display: block; margin-bottom: 4px; }
  .ff-val { font-family: 'Syne', sans-serif; font-size: 22px; font-weight: 800; }
  /* SIDE DETAIL */
  ...
  .sd-type { font-family: 'DM Mono', monospace; font-size: 9px; letter-spacing: 2px; color: #8b949e; text-transform: uppercase; margin-bottom: 5px; }
  .sd-site { font-family: 'Syne', sans-serif; font-size: 16px; font-weight: 800; color: #f0f6fc; line-height: 1.2; margin-bottom: 8px; }
  ...
  .kv-k { font-family: 'DM Mono', monospace; font-size: 9px; color: #8b949e; text-transform: uppercase; letter-spacing: 1px; }
  .kv-v { font-size: 12px; color: #c9d1d9; font-weight: 500; }
  .sd-notes { margin-top: 10px; padding: 8px 10px; background: #161b22; border: 1px solid #21262d; border-radius: 5px; }
  .sdn-lbl { font-family: 'DM Mono', monospace; font-size: 9px; color: #8b949e; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 3px; }
  .sdn-txt { font-size: 11px; color: #8b949e; line-height: 1.6; }
  .sd-actions { display: flex; gap: 5px; margin-top: 11px; flex-wrap: wrap; }
  .read-only-tag { margin-top: 11px; font-size: 10px; color: #8b949e; background: #21262d; padding: 5px 10px; border-radius: 4px; text-align: center; font-family: 'DM Mono', monospace; letter-spacing: 1px; }
  /* FLEET */
  .fleet-summary { padding: 12px; }
  .fs-title { font-family: 'DM Mono', monospace; font-size: 9px; font-weight: 500; letter-spacing: 2px; color: #8b949e; text-transform: uppercase; margin-bottom: 10px; }
  ...
  .tag { font-family: 'DM Mono', monospace; font-size: 9px; padding: 1px 5px; border-radius: 3px; font-weight: 500; }
  ...
  .modal-hdr { display: flex; align-items: center; justify-content: space-between; padding: 14px 18px; border-bottom: 1px solid #21262d; font-family: 'Syne', sans-serif; font-size: 17px; font-weight: 800; color: #f0f6fc; }
  ...
  .mfld label { font-family: 'DM Mono', monospace; font-size: 9px; font-weight: 500; letter-spacing: 2px; text-transform: uppercase; color: #8b949e; }
  .mfld input, .mfld select, .mfld textarea { background: #0d1117; border: 1px solid #30363d; border-radius: 5px; padding: 7px 10px; color: #c9d1d9; font-family: 'DM Sans', sans-serif; font-size: 12px; outline: none; width: 100%; transition: border-color .15s; }
  ...
  .umc-av { width: 42px; height: 42px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-family: 'Syne', sans-serif; font-size: 14px; font-weight: 800; flex-shrink: 0; border: 2px solid; }
  .umc-name { font-family: 'Syne', sans-serif; font-size: 14px; font-weight: 700; color: #f0f6fc; }
  ...
  .up { font-family: 'DM Mono', monospace; font-size: 9px; font-weight: 500; padding: 2px 7px; border-radius: 3px; }
  ...
  .umt-lbl { font-family: 'DM Mono', monospace; font-size: 9px; color: #8b949e; letter-spacing: 1px; }
